...
author: 
  ...
  - Dingding Wang
  - Xueru Ma
thanks: "Code and data are available at: https://github.com/chenme72/Women-and-Girls-Education-The-Unequal-Balance.git"
output:
...
clean2<- clean %>%
  rename(college = college_or_higher, 
         missing = dont_know_or_missing,
         female = number_of_females,
         med = median_number_of_years_of_schooling)

#install.packages("kableExtra")
library(kableExtra)
library(knitr)
kable(clean2[1:5,], caption = "Clean Dataset") %>%
...
demography_data_age <- tibble(all = data_with_age) %>% 
  ...
  mutate(all = str_squish(all)) %>% separate(col = all,
           into = c("background", "none", "elementary", "high_school", "college", "missing", "total", "females","med"),
           sep = " ", 
           remove = TRUE,
           fill = "right",
           extra = "drop") 
...
demography_data_age<- demography_data_age %>%
  filter(!is.na(background) & !is.na( none)  & !is.na( high_school) & !is.na(college) & !is.na(missing)& !is.na(total)  & !is.na(females) & !is.na(med)) %>% 
  mutate_all(~str_remove_all(.,",")) %>% 
  mutate(character = "age") %>%
  select("background", "none", "elementary", "high_school", "college", "missing", "total", "females","med") 
   

demography_data_residence <- demography_data_residence %>%
  filter(!is.na(background) & !is.na( none)  & !is.na( high_school) & !is.na(college) & !is.na(missing)& !is.na(total)  & !is.na( females) & !is.na(med)) %>% 
  mutate_all(~str_remove_all(.,",")) %>%
  mutate(character = "residence") %>%
  select("background", "none", "elementary", "high_school", "college", "missing", "total", "females","med","character") 
   

demography_data_region <- demography_data_region %>%
  filter(!is.na(background) & !is.na( none)  & !is.na( high_school) & !is.na(college) & !is.na(missing)& !is.na(total)  & !is.na( females) & !is.na(med)) %>% 
  mutate_all(~str_remove_all(.,",")) %>% 
  mutate(character = "region") %>%
  select("background", "none", "elementary", "high_school", "college", "missing", "total", "females","med","character") 
...
2. *Was the "raw" data saved in addition to the preprocessed/cleaned/labeled data (for example, to support unanticipated future uses)? If so, please provide a link or other access point to the "raw" data.*
	- Yes, both the clean and "raw" data are saved and available at the link:https://github.com/chenme72/Women-and-Girls-Education-The-Unequal-Balance.git
...
2. *Is there a repository that links to any or all papers or systems that use the dataset? If so, please provide a link or other access point.*
	- The repository containing all the files pertaining to the dataset is available at https://github.com/chenme72/Women-and-Girls-Education-The-Unequal-Balance.git
...
2. *How will the dataset be distributed (for example, tarball on website, API, GitHub)? Does the dataset have a digital object identifier (DOI)?*
	- The dataset can be obtained by running the r script "01-gather_data.R" available at the link: https://github.com/chenme72/Women-and-Girls-Education-The-Unequal-Balance.git
...
4. *Will the dataset be updated (for example, to correct labeling errors, add new instances, delete instances)? If so, please describe how often, by whom, and how updates will be communicated to dataset consumers (for example, mailing list, GitHub)?*
	- If there is any error made in the creation of the dataset, the dataset will be updated by Dingding Wang. Updates will be communicated as a note in the Readme file at the link below: https://github.com/chenme72/Women-and-Girls-Education-The-Unequal-Balance.git
